...
# Your instruction
instruction = 'Open the YouTube app, search for the "Hung-yi Lee" channel, play its latest video.'
...
def get_perception_infos(adb_path, screenshot_file):
    ...
    perception_infos = []  # 上方对屏幕信息做了OCR，结果保存到这个列表里
    for i in range(len(coordinates)):
        perception_info = {"text": "text: " + text[i], "coordinates": coordinates[i]}
        perception_infos.append(perception_info)
        
    coordinates = det(screenshot_file, "icon", groundingdino_model)  # 屏幕图标的检测，保存到info里
    ...
    return perception_infos, width, height  # 获取到的信息和手机的宽高
...
while True:
    ...
    output_action = inference_chat(chat_action, API_url, 'gpt-4o', token)
    # 模型当前的思考
    ...
    if memory_switch:
        ...
        output_memory = inference_chat(chat_action, API_url, 'gpt-4o', token)
    ...
    if reflection_switch:
        ...
        output_reflect = inference_chat(chat_reflect, API_url, 'gpt-4o', token)
        ...
        if 'A' in reflect:  # A 表示符合预期
            ...
            output_planning = inference_chat(chat_planning, API_url, 'gpt-4o', token)
    ...
    os.remove(last_screenshot_file)  # 把这一轮的图片删除，开始下一轮
